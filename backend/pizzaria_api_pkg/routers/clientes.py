...
def adicionar_pizza_ao_carrinho(
    ...
    """✅ CORRIGIDO: Valida propriedade da pizza antes de adicionar"""
    cliente = db.query(ClienteModel).filter(ClienteModel.usuario_id == usuario["usuario_id"]).first()
    if not cliente:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
    
    # ✅ VALIDAR SE A PIZZA EXISTE
    pizza = db.query(PizzaPersonalizada).filter(
        PizzaPersonalizada.id == dados.pizza_id
    ).first()
    
    if not pizza:
        raise HTTPException(status_code=404, detail="Pizza personalizada não encontrada")
    
    # ✅ VALIDAR SE A PIZZA PERTENCE AO CLIENTE
    if pizza.cliente_id != cliente.id:
        raise HTTPException(
            status_code=403, 
            detail="Você só pode adicionar suas próprias pizzas personalizadas ao carrinho"
        )
    
    # ✅ VERIFICAR SE JÁ EXISTE NO CARRINHO
    item_existente = db.query(Carrinho).filter(
        Carrinho.cliente_id == cliente.id,
        Carrinho.produto_personalizado_id == dados.pizza_id
    ).first()
    ...
    return {
        "msg": "Pizza adicionada ao carrinho com sucesso",
        "carrinho_id": novo_item.id,
